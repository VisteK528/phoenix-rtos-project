    ...
    for (;;) {
        ...
        if (err < 0) {
            if (err == -EINTR) {
                ...
                fprintf(stderr, "decrypt_server: msgRecv returned %d (%s)\r\n", err, strerror(err));
                exit(EXIT_FAILURE);
            }
        }
...
int main(void)
{
	...
	if (portCreate(&oid.port) < 0) {
		fprintf(stderr, "decrypt_server: portCreate failed\n");
		return EXIT_FAILURE;
	}
	...
	if (create_dev(&oid, "decrypt_input") < 0) {
		fprintf(stderr, "decrypt_server: create_dev failed\n");
		return EXIT_FAILURE;
	}

	if (create_dev(&oid2, "decrypt_output") < 0) {
		fprintf(stderr, "decrypt_server: create_dev failed\n");
		return EXIT_FAILURE;
	}
